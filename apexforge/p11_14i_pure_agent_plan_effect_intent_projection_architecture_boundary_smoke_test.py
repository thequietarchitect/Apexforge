"""P11.14I pure AgentPlan effect-intent projection architecture boundary."""
...
BASELINE = "f7faccd051e0101e43cecf564aeefcc5e45ffa6d"
PREDECESSOR_TAG = "afp-p11-14h-freeze"
EXPECTED_BRANCH = "p11-14i-pure-agent-plan-effect-intent-projection-architecture-boundary"
...
FROZEN_HASHES = {
    ...
    "apexforge/p11_14h_agent_plan_consumer_action_generalization_architecture_boundary_smoke_test.py":
        "673105BFD142CAA2F7F23A7DDDDC901495A6B144C04AD91F8DF007BB237FE17C",
    "docs/p11/P11_14H_AGENT_PLAN_CONSUMER_ACTION_GENERALIZATION_ARCHITECTURE_BOUNDARY.md":
        "0859C00B8773EEABA55455D3AAF163B8D0C93E0B5F60DED595E9E42B456B8A9A",
...
def main() -> int:
    ...
    require(resolved.stdout.strip() == BASELINE, "P11.14H freeze target changed")
    require(
        git("merge-base", "--is-ancestor", PREDECESSOR_TAG, "HEAD").returncode == 0,
        "P11.14H freeze is not ancestor of P11.14I",
    )

    branch = git("branch", "--show-current")
    require(branch.returncode == 0, branch.stderr.strip())
    require(branch.stdout.strip() == EXPECTED_BRANCH, "P11.14I branch changed")
    ...
    require(
        ...
        "P11.14I unexpectedly changed production agent module set: {!r}".format(
    ...
    for name in FORBIDDEN_I_PRODUCTION_NAMES:
        require(
            name not in production_text,
            "premature I production surface leaked: {}".format(name),
        )

    require(
        not (agents_dir / "plan_projection.py").exists(),
        "P11.14I must remain architecture-only; plan_projection.py exists",
    )
    require(
    ...
    effects = importlib.import_module("effects.model")
    require(hasattr(effects, "EffectIntent"), "EffectIntent ownership changed")

    authority_validator = (
        ROOT / "apexforge" / "authority" / "validator.py"
    ).read_text(encoding="utf-8")
    require(
        "def authorize_principal_capabilities(" in authority_validator,
        "capability authorization ownership changed",
    )

    runtime_state = (
        ROOT / "apexforge" / "runtime" / "state.py"
    ).read_text(encoding="utf-8")
    require("class StateDelta" in runtime_state, "StateDelta ownership changed")
    ...
    require(
        ...
        "EffectIntent passive boundary changed",
    )

    print("P11_14H_FREEZE_ANCESTRY=PASS")
    print("P11_14I_ARCHITECTURE_ONLY=PASS")
    print("P11_14I_PRODUCTION_FILE_CHANGES=NONE")
    print("FIRST_AGENT_PLAN_CONSUMER=PURE_EFFECT_INTENT_PROJECTION")
    print("P11_14J_OWNER=agents.plan_projection")
    ...
    print("PROJECTION_OUTPUT_IDENTITY=PRESERVED")
    print("EMPTY_PLAN_PROJECTION=VALID")
    ...
    print("STATE_DELTA=UNTOUCHED")
    print("AGENTS_EXECUTION=DEFERRED")
    print("AGENT_ACTION=DEFERRED")
    print("P11_14I_PURE_AGENT_PLAN_EFFECT_INTENT_PROJECTION_ARCHITECTURE_BOUNDARY=PASS")
    print("P11_14I_EXIT=0")
    return 0
