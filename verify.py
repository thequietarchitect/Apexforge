...
from air.model import AIRProgram, VerificationResult, validate_assignment_shape, validate_state_definition_shape
...
class AIRVerifier:
    def verify(self, program: AIRProgram) -> VerificationResult:
        ...
        if program.version != AIR_VERSION:
            append_diagnostic(diagnostics, "error", "AIR001", f"unsupported AIR version: {program.version}")
        ...
        for state in program.states:
            if not validate_state_definition_shape(state):
                append_diagnostic(diagnostics, "error", "AIR010", "state must be an int state with an int initial value", state.id)

        for check in program.authority_checks:
            if check.principal not in principals:
                append_diagnostic(diagnostics, "error", "AIR020", f"authority principal does not exist: {check.principal}", check.id)

        for decision in program.causal_decisions:
            if decision.policy != "max_weight":
                append_diagnostic(diagnostics, "error", "AIR030", f"unsupported causal policy: {decision.policy}", decision.id)
            if not decision.paths:
                append_diagnostic(diagnostics, "error", "AIR031", "causal decision must have at least one path", decision.id)
            self._check_unique(f"path in {decision.id}", decision.paths, diagnostics)
            for path in decision.paths:
                if not is_int(path.weight):
                    append_diagnostic(diagnostics, "error", "AIR032", "causal path weight must be an int", path.id)
                for assignment in path.assignments:
                    if assignment.state not in states:
                        append_diagnostic(diagnostics, "error", "AIR033", f"assignment state does not exist: {assignment.state}", path.id)
                    if not validate_assignment_shape(assignment):
                        append_diagnostic(diagnostics, "error", "AIR034", "assignment operation/value shape is invalid", path.id)
                for emission in path.emits:
                    if emission.event not in events:
                        append_diagnostic(diagnostics, "error", "AIR036", f"event does not exist: {emission.event}", path.id)

        for directive in program.directives:
            if directive.principal not in principals:
                append_diagnostic(diagnostics, "error", "AIR040", f"directive principal does not exist: {directive.principal}", directive.id)
            if not is_int(directive.order):
                append_diagnostic(diagnostics, "error", "AIR041", "directive order must be an int", directive.id)
            for check_id in directive.authority_checks:
                if check_id not in checks:
                    append_diagnostic(diagnostics, "error", "AIR042", f"authority check does not exist: {check_id}", directive.id)
                    continue
            ...
            for decision_id in directive.causal_decisions:
                if decision_id not in decisions:
                    append_diagnostic(diagnostics, "error", "AIR044", f"causal decision does not exist: {decision_id}", directive.id)

        return VerificationResult(program, tuple(sorted(diagnostics)))

    @staticmethod
    def _check_unique(label: str, items: Iterable[HasId], diagnostics: list[Diagnostic]) -> None:
        seen: set[str] = set()
        for item in items:
            if item.id in seen:
                append_diagnostic(diagnostics, "error", "AIR000", f"duplicate {label} id: {item.id}", item.id)
            seen.add(item.id)
